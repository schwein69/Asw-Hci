...
onMounted(() => {
  ...
  if (!resetToken.value || resetToken.value.trim() === '') {
    errorMessage.value = 'Invalid reset link. Please check the link from your email.';
  ...
  <div class="min-h-screen flex items-center justify-center p-4 transition-colors duration-300">
    ...
    <div class="w-full max-w-sm p-8">
      ...
      <div class="mb-8 text-center">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-white tracking-tight mb-2">
          Reset Password
        </h1>
      ...
      <div v-if="errorMessage && !isInitializing" class="alert alert-error mb-4 rounded-full shadow-md">
        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
      ...
      <div v-if="showForm && !isInitializing" class="form-control w-full space-y-4">
        <div>
          <input 
      ...
      <div v-if="!showForm && !isInitializing && errorMessage" class="flex justify-center mt-6">
        <button 
          @click="router.push('/login')"
          class="btn btn-sm btn-ghost text-success rounded-full no-animation active:scale-95 h-11"
        >
